...
Introduce yourself, give the name of your PhD project and name your advisor. Introduce the research as predicting changes in extreme sub-daily precipitation. Let the poster carry the humour; keep the introduction in your own voice.
:::
...
![](design-studies/designs/hessian-hill-matrix.png){.hessian-overlay .fragment .nostretch fig-alt="A two-by-two Hessian labelled Level and Spread, with dark diagonal cells and grey off-diagonal cells."}

::: {.notes}
**The Hessian: the shape near the best fit — 35 seconds.**

The peak is the best fit. Explain how the fit changes when one parameter moves with the other fixed, then how the two can compensate when they move together. Use the hill as a visual cue; keep the explanation in your own words.

Click to reveal the matrix. Diagonal: how tightly each parameter is pinned down, holding the other fixed. Off-diagonal: interaction between the estimates. The full matrix describes their joint uncertainty. Two parameters give us two-by-two; our nine estimates give us nine-by-nine. This local shape builds the normal approximation.

Preparation detail, not spoken: the illustration is a toy relative likelihood over centred, rescaled Level and Spread coordinates, holding tail fixed. The relevant Hessian is that of the log-likelihood at its peak; its negative is the precision for the normal approximation. Because this relative likelihood equals one at the peak and has zero gradient there, its Hessian at the peak equals the log-likelihood Hessian. A diagonal entry of the precision matrix is conditional precision with other parameters fixed, not the inverse marginal variance. The covariance of the normal approximation is the inverse of the full precision matrix. Curvature in a joint direction combines diagonal and off-diagonal terms. Cell colours identify matrix positions, not numerical signs, magnitudes or correlations.
:::
