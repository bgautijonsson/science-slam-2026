...
::: {.notes}
**A light introduction — 10 seconds.**

Introduce yourself and give the name of your PhD project in your own words. This opening can stay on screen while you settle in. Introduce the topic lightly: the wettest hour of the year. Advance to introduce Birgir, then save the full comic title for the Strangelove poster that follows.
:::
...
::: {.notes}
**Meet my advisor — 8 seconds.**

Introduce Birgir Hrafnkelsson as your PhD advisor in your own words. Give the audience a moment with the cartoon; no need to explain latent variables here. Advance once when you are ready to reveal the Strangelove poster to everyone together.

Preparation note: this introduction establishes Birgir's appearance before he returns in the final poster, as identified by the author. Let that later recognition happen naturally. Author-supplied birgir.png, added 24 September 2026; full image retained without cropping.
:::

## {#poster .art background-image="Figures/bomb.jpg" background-size="contain" background-color="#000000"}

::: {.notes}
**The poster reveal — 5 seconds.**

Reveal the full poster and comic title together. Give the audience a moment to take it in and let the joke land before moving on. The self-introduction and advisor introduction have their own slides, so the poster gets its own beat. Then move to the rainfall photograph and why predicting changes matters.
:::
...
Start with all nine estimates visible. Click once to focus on Level (location), twice for Spread (scale), and three times for Tail (shape). Each step fades the other two columns while keeping the station names and rainfall records visible. The fourth click advances to the thinking interlude; the left arrow reverses the steps.
...
Transition: the thinking interlude asks where these estimates come from and brings back the promised Hessian. After Gauss, zoom in on level and spread at one station, holding tail fixed. Keep A-B-C and L-S-T in this order for the later nine-by-nine grid.
:::

## {#thinking .art background-image="Figures/binni-thinking.png" background-size="contain" background-color="#f8f6f0"}

::: {.notes}
**Where is the Hessian? — 4 seconds.**

“But how do we get these estimates? And where’s the Hessian I promised you?”

Keep the slide image-only. Give the image a beat, then click to reveal Gauss. Treat the two images as one quick setup and payoff; the hill immediately afterwards provides the explanation. The supplied artwork is shown in full, without cropping.
:::
...
The Carl Weathers image starts the recipe: “You take an assumption of independence”. The caption replaces the old heading; introduce the people analogy underneath it.
...
The next ingredient: “You add within-station dependence”. Keep the same image and let the changed caption carry the build. In the explanation, make clear that we are relaxing the earlier independence assumption, not retaining independence and dependence for the same pair.
...
The next ingredient: “You add spatial dependence between neighbours”. Tie this directly to the newly blue bridges and matrix blocks.

Now the groups share openly with their neighbours. The same storm can affect several stations, so the likelihood couples estimates across station boundaries. Point from one blue bridge between groups to its blue off-diagonal block and the matching block across the diagonal: a single connection is represented symmetrically in the matrix. This is the part I work on: carrying those connections into an approximation we can compute with. Let the recipe joke build towards the following slide; keep this explanation focused on the newly blue connections.
...
::: {.notes}
**Back to the research, then Birgir's lesson — 25 seconds.**

“Baby, you got a stew going”. Give the payoff a beat before connecting the matrix to the map. This completes the four-slide recipe; no extra click is needed.

With more stations, the matrix gets much bigger, but the idea is the same: keep the connections. The estimates and their Hessian feed into a spatial model, giving us a picture of how a rainfall parameter varies from place to place. Gesture from the repeated black and blue blocks to the map. This map illustrates the rainfall level; the other parameters can also have spatial fields.

Keep the map onscreen for the setup, in your own words: “During my PhD, whenever a model gets so complicated that I start wondering how on earth I’m going to fit it, I come back to the most important lesson I’ve learned from Birgir…” Pause, then click to reveal his answer.

Preparation detail, not spoken: both panels are illustrative. The 90-by-90 matrix represents thirty fictional stations with three parameters each; this is not the project's actual station count or geographical neighbour graph. It extends the earlier chain pattern, with symmetric within-station blocks and neighbouring-station blocks. Its negative is a strictly diagonally dominant, positive-definite toy precision matrix. The normal approximation combines the estimates and their uncertainty; the Hessian alone does not determine a spatial mean or prediction. The arrow summarises the subsequent spatial modelling step, not a direct matrix-to-map calculation.

The UK map, including Northern Ireland, shows an arbitrary smooth field of relative rainfall level generated from fixed random Gaussian basis weights. It is not a fitted result, a rainfall forecast, or a simulation from the displayed Hessian. Shading means lower to higher level, not uncertainty. The Republic of Ireland is faint geographical context without field values. The reference image supplied by the author showed Ireland; this slide follows the requested UK geography. Coastlines: Natural Earth 1:50m, public-domain data bundled with rnaturalearthdata; British National Grid projection preserves the map's physical aspect.
:::

## {#birgir-lesson .art background-image="Figures/birgir-hessian.png" background-size="contain" background-color="#f8f6f0"}

::: {.notes}
**Birgir's most important lesson — 5 seconds.**

Let the image deliver the punchline. Hold for the audience to read it and laugh; add no heading or explanation. Birgir returns from the advisor introduction. Then click to the final poster for the farewell. Author-supplied artwork, birgir_hessian.png, shown in full without cropping.
:::

## {#callback .art background-image="Figures/hessian-ending.png" background-size="contain" background-color="#f8f6f0"}

::: {.notes}
**The final farewell — 5 seconds.**

Reveal the closing poster, thank the audience, and stop. Birgir's lesson is the spoken payoff; this image is the farewell. Birgir, introduced on slide two, returns here alongside the other riders. Do not introduce another scientific point. Author-supplied artwork, hessian_ending.png, added 24 September 2026; the complete image is shown without cropping.
:::
