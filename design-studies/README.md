...
- The three station-parameter reveals omit the shared line-type legend and bottom explanatory captions; `../index.qmd` keeps those explanations in the speaker notes. Curves, axes and parameter labels retain their positions and scale.
- The nine-estimates slide omits “Lower → higher”. Three exclusive native fragments fade the two unselected columns in Level → Spread → Tail order, aligned with the fixed 70%, 81%, 92% column centres. Mouse clicks and forward/backward keys step through the comparisons; the following click advances to the thinking interlude.
- `../Figures/binni-thinking.png`: author-supplied image, copied unchanged. An image-only interlude between the estimates and Gauss asks where the estimates come from and recalls the promised Hessian. Both interludes advance on a mouse click.
- `../Figures/birgir-hessian.png`: author-supplied image, copied unchanged. The research map holds the spoken setup about Birgir's most important lesson; a click reveals the image-only punchline, then another click reveals the existing closing poster for the farewell.
...
- The four live matrix/research slides add `../Figures/stew.png` and the author's exact recipe captions as italic quotations in a shared HTML header (`../index.qmd`, `../theme.scss`). The supplied screenshot is copied unchanged. The figure generators leave the header clear and shift the full-size diagrams down by 4.5% of slide height. These captions replace the earlier titles. The research slide omits panel headings, leaving the spatial-model arrow, map legend and illustrative-example label to support the spoken explanation.
- `research-bridge.R`: generates `designs/research-bridge.{png,svg}` for the final research bridge. A 90×90 toy Hessian extends the same three-parameter station blocks and chain links; its negative is checked positive definite. The UK rainfall-level map is an independent illustrative smooth field, built from fixed Gaussian radial-basis weights and masked to Natural Earth 1:50m UK boundaries (public domain, bundled with `rnaturalearthdata`). Northern Ireland and UK islands are included; the Republic of Ireland is unshaded context. The map uses British National Grid coordinates with equal physical scales and a lower-to-higher grey legend. Neither panel is a fitted result, and the map is not calculated directly from the Hessian.
...
LC_ALL=en_US.UTF-8 Rscript research-bridge.R
python3 scripts/package_draft.py
quarto render
python3 scripts/build_design_board.py
```

R packages: grid, ragg, svglite and png; the research bridge also uses sf, rnaturalearth and rnaturalearthdata; the Python builders use only the standard library. Figure text uses Lato; PNGs preserve the rendered appearance, while SVG text needs the font installed. `slide-designs.R` and `hessian-hill.R` explicitly register Lato from the macOS user font directory when available and stop if Lato cannot be resolved, preventing silent font substitution. The local Quarto configuration keeps these draft renders separate from the repository's main deck render. The HTML files embed their images and presentation assets.
